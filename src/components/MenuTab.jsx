...
export default function MenuTab({ menuAction, setMenuAction }) {
    ...
    const [newCat, setNewCat] = useState({ name_th: '', name_en: '' });
    ...
    const getItemCatId = (item) => item?.cat ?? item?.category_id ?? '';
    const getCategoryNameTH = (category) => category?.name_th || category?.name_en || '';
    const getCategoryNameEN = (category) => category?.name_en || '';

    useEffect(() => {
        if (menuAction === 'addCategory') {
            setEditingCat(null);
            setNewCat({ name_th: '', name_en: '' });
            setIsCatModalOpen(true);
            setMenuAction(null);
        } else if (menuAction === 'addItem') {
            setEditingItem(null);
            setNewItem({
                name_th: '',
                name_en: '',
                price: '',
                cat: activeCatFilter !== 'all' ? activeCatFilter : categories[0]?.id || '',
                color: 'bg-stone-200',
                image: '',
                options: [],
            });
    ...
    const filteredItems = menuItems.filter((m) => {
        const catId = getItemCatId(m);

        const categoryMatch =
            activeCatFilter === 'all' || String(catId) === String(activeCatFilter);
    ...
    const saveCategory = async () => {
        ...
        const payload = {
            ...
            name_en: newCat.name_en.trim() || newCat.name_th.trim(),
        };
    ...
    const saveMenuItem = async () => {
        ...
        const payload = {
            ...
            category_id: newItem.cat === '' ? null : Number(newItem.cat),
            image: newItem.image,
            color: newItem.color,
        };
        ...
        try {
            if (editingItem) {
                ...
                setMenuItems([
                    {
                        ...created,
                        cat: created.category_id,
                        name_th: created.name_th,
    ...
    const handleDropItem = (idx) => {
        ...
        const v = n.filter(
            (m) =>
                activeCatFilter === 'all' ||
                String(getItemCatId(m)) === String(activeCatFilter)
        );

        const d = n.findIndex((m) => m.id === v[draggedIdx].id);
        const t = n.findIndex((m) => m.id === v[idx].id);

        const [m] = n.splice(d, 1);
        n.splice(t, 0, m);
            ...
            {isCatModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[2.5rem] p-6 lg:p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
                        <h3 className="font-black text-xl lg:text-2xl mb-6 text-stone-800">
                            {editingCat ? 'แก้ไขหมวดหมู่' : 'เพิ่มหมวดหมู่'}
                        </h3>
                        ...
                        <div className="space-y-3 mb-8">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setNewCat({ ...newCat, name_th: e.target.value })
                                    }
                                    className="w-full p-4 border-2 border-stone-100 bg-stone-50 rounded-2xl text-center font-bold text-lg outline-none focus:border-[#861b00]"
                                    placeholder="เช่น ☕️ กาแฟ"
                                />
                            </div>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 py-3 lg:py-4 bg-stone-100 text-stone-500 font-bold rounded-2xl"
                            >
                                ยกเลิก
                            </button>
                            <button
                                onClick={saveCategory}
                                className="flex-[2] py-3 lg:py-4 bg-stone-800 text-white font-black rounded-2xl"
                            >
                                บันทึก
                            </button>
                        </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[2.5rem] p-6 lg:p-8 max-w-lg w-full shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="flex justify-between items-center mb-5 shrink-0 border-b pb-4">
                            <h3 className="font-black text-xl lg:text-2xl text-stone-800">
                                {editingItem ? 'แก้ไขเมนู' : 'เพิ่มเมนูใหม่'}
                            </h3>

                            <button
                                onClick={() => setIsItemModalOpen(false)}
                                className="w-8 h-8 bg-stone-100 text-stone-400 rounded-full flex items-center justify-center"
                            >
                        ...
                        <div className="space-y-4 flex-1 overflow-y-auto no-scrollbar pr-2 pb-4">
                            <div className="flex gap-4 items-center bg-stone-50 border p-4 rounded-2xl">
                                    ...
                                    {newItem.image ? (
                                        <>
                                            ...
                                            <button
                                                ...
                                                className="absolute inset-0 bg-black/50 text-white flex items-center justify-center opacity-0 group-hover/preview:opacity-100"
                                            >
                                ...
                                <div className="flex-1 space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full text-xs text-stone-500 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[11px] file:font-black file:bg-[#861b00] file:text-white cursor-pointer bg-white border rounded-xl"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full p-3 border-2 border-stone-100 bg-stone-50 rounded-xl font-bold outline-none focus:border-[#861b00]"
                                    />
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full p-3 border-2 border-stone-100 bg-stone-50 rounded-xl font-black text-[#861b00] outline-none"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full p-3 border-2 border-stone-100 bg-stone-50 rounded-xl font-bold outline-none text-sm"
                                    >
                                        {categories.map((c) => (
                            ...
                            <div className="pt-3 border-t">
                                <label className="text-[10px] font-bold text-stone-500 mb-2 block uppercase">
                                    สีประจำเมนู
                                </label>
                        ...
                        <div className="flex gap-3 shrink-0 pt-4 border-t">
                            <button
                                onClick={() => setIsItemModalOpen(false)}
                                className="flex-1 py-3.5 bg-stone-100 text-stone-500 font-bold rounded-xl"
                            >
                                ยกเลิก
                            </button>
                            <button
                                onClick={saveMenuItem}
                                className="flex-[2] py-3.5 bg-[#861b00] text-white font-black rounded-xl"
                            >
                                บันทึก
                            </button>
                        </div>
                    ...
                    <div className="bg-white rounded-[2.5rem] p-6 lg:p-8 max-w-2xl w-full shadow-2xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="flex-1 overflow-y-auto no-scrollbar space-y-4 pr-2 pb-4">
                            {optionGroups.length === 0 ? (
                                ...
                                optionGroups.map((group, idx) => (
                                    <div
                                        key={idx}
                                        className="bg-white border-2 border-stone-100 p-4 rounded-2xl flex flex-col sm:flex-row gap-4 items-start sm:items-center"
                                    >
                                        <div className="flex-1">
                                            <h4 className="font-black text-stone-800 text-lg mb-1">
                                                {group.name_th}
                                            </h4>

                                            <p className="text-xs font-bold text-stone-400">
                                                ใช้กับ:{' '}
                                                {group.applyTo?.join(', ') || 'ทั้งหมด'}
                                            </p>
                                            ...
                                            <div className="mt-2 flex flex-wrap gap-2">
                                                {group.choices
                                                    ...
                                                    .map((c, i) => (
                                                        <span
                                                            key={i}
                                                            className="bg-stone-100 text-stone-600 px-2 py-1 rounded text-[10px] font-bold"
                                                        >
                                                            {c.n}{' '}
                                                            {c.p > 0
                                                                ? `(+฿${c.p})`
                                        ...
                                        <div className="flex gap-2 w-full sm:w-auto">
                                            <button
                                                ...
                                                className="flex-1 sm:flex-none px-4 py-2 bg-amber-50 text-amber-600 rounded-xl font-bold text-xs hover:bg-amber-500 hover:text-white transition-colors"
                                            >
                                                แก้ไข
                                            </button>
                                            ...
                                            <button
                                                ...
                                                className="flex-1 sm:flex-none px-4 py-2 bg-red-50 text-red-600 rounded-xl font-bold text-xs hover:bg-red-500 hover:text-white transition-colors"
                                            >
                                                ลบ
                                            </button>
                                        </div>
                        ...
                        <div className="pt-4 border-t shrink-0">
                            <button
                                ...
                                className="w-full py-4 bg-stone-800 hover:bg-black text-white font-black rounded-2xl transition-colors flex items-center justify-center gap-2"
                            >
            ...
            {isCreateGroupModalOpen && (
                <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-[2.5rem] p-6 lg:p-8 max-w-md w-full shadow-2xl flex flex-col max-h-[90vh]">
                        <h3 className="font-black text-xl text-stone-800 mb-4">
                            {editingOptionGroup
                                ? 'แก้ไขกลุ่มตัวเลือก'
                                : 'สร้างกลุ่มตัวเลือกใหม่'}
                        </h3>

                        <div className="space-y-4 overflow-y-auto no-scrollbar pr-2 pb-4">
                            <div>
                                <label className="text-[10px] font-bold text-stone-400 uppercase">
                                    ชื่อกลุ่ม เช่น ระดับความหวาน
                                </label>
                                <input
                                    value={newOptionGroup.name_th}
                                    onChange={(e) =>
                                        setNewOptionGroup({
                                            ...newOptionGroup,
                                            name_th: e.target.value,
                                    ...
                                    className="w-full p-3 border-2 border-stone-100 bg-stone-50 rounded-xl font-bold outline-none focus:border-amber-500"
                            ...
                            <div>
                                    ...
                                    {newOptionGroup.choices.map((choice, idx) => (
                                        <div key={idx} className="flex gap-2">
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    const nc = [
                                                        ...newOptionGroup.choices,
                                                    ];
                                                    nc[idx].n = e.target.value;
                                                    setNewOptionGroup({
                                                        ...newOptionGroup,
                                                        choices: nc,
                                                    });
                                                }}
                                                className="flex-[2] p-2.5 border-2 border-stone-100 bg-white rounded-xl font-bold text-xs outline-none focus:border-amber-500"
                                                placeholder="ชื่อตัวเลือก เช่น หวาน 50%"
                                            />
                                            ...
                                            <div className="flex-[1] relative">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        nc[idx].p = e.target.value;
                                                        setNewOptionGroup({
                                                            ...newOptionGroup,
                                                            choices: nc,
                                                        });
                                                    }}
                                                    className="w-full p-2.5 pl-6 border-2 border-stone-100 bg-white rounded-xl font-bold text-xs outline-none focus:border-amber-500"
                                ...
                                <button
                                    ...
                                    className="text-xs font-bold text-amber-600 mt-2 flex items-center gap-1 hover:text-amber-700"
                                >
                                    <span className="material-symbols-outlined text-[14px]">
                                        add
                                    </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-8 flex-1 min-h-0 overflow-hidden">
                <div className="md:col-span-1 bg-white p-4 lg:p-6 rounded-[2.5rem] border border-stone-200 shadow-sm flex flex-col h-full overflow-hidden">
                    <div className="flex flex-col mb-4 border-b pb-4 shrink-0">
                        <span className="font-bold text-[10px] uppercase tracking-widest text-stone-400 mb-2">
                            ค้นหาหมวดหมู่
                        </span>

                        <div className="relative">
                            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-stone-300 text-sm">
                                search
                            </span>
                            <input
                                type="text"
                                ...
                                placeholder="พิมพ์..."
                                className="w-full pl-9 pr-4 py-2 bg-stone-50 border rounded-xl text-xs font-bold outline-none focus:border-amber-500 transition-all"
                            />
                        </div>
                    </div>
                                ...
                                >
                                    ...
                                    <div className="flex gap-1 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 shrink-0">
                                        <button
                                            ...
                                            className="w-7 h-7 rounded-full text-emerald-500 hover:bg-emerald-50 flex items-center justify-center border bg-white shadow-sm"
                                        >
                                            <span className="material-symbols-outlined text-[14px]">
                                                edit
                                            </span>
                                        ...
                                        <button
                                            ...
                                            className="w-7 h-7 rounded-full text-stone-300 hover:text-red-500 flex items-center justify-center border bg-white shadow-sm"
                                        >
                                            <span className="material-symbols-outlined text-[14px]">
                                                delete
                                            </span>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 border-b pb-4 shrink-0">
                        ...
                        <div className="relative w-full sm:w-56 shrink-0">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2 bg-white border border-stone-200 rounded-full text-xs font-bold outline-none focus:border-[#861b00] shadow-sm"
                            />
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left min-w-[550px] lg:min-w-full">
                            <thead className="bg-stone-50 border-b text-[10px] font-bold text-stone-500 uppercase tracking-widest sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="py-4 pr-6 text-center w-24 lg:w-28">
                                        จัดการ
                                    </th>
                                </tr>
                            </thead>
                                    ...
                                    return (
                                        ...
                                        >
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    >
                                                        {it.image ? (
                                                            <img
                                                                src={it.image}
                                                                className="w-full h-full object-cover"
                                                                alt={it.name_th || it.name_en || '-'}
                                                            />
                                                        ) : (
                                                            <span className="font-black text-black/30 text-[11px] lg:text-xs uppercase">
                                                                {it.name_th?.charAt(0) ||
                                                                    '?'}
                                                            </span>
                                                        )}
                                                    ...
                                                    <div className="flex flex-col min-w-0">
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-black text-stone-800 text-[13px] lg:text-[14px] truncate">
                                                                {it.name_th || it.name_en || '-'}
                                                            </span>
                                                        ...
                                                        {it.name_en && (
                                                            <span className="font-bold text-stone-400 text-[10px] lg:text-[11px] truncate">
                                                                {it.name_en}
                                                            </span>
                                                        )}

                                                        <div className="hidden lg:block mt-1">
                                                            <span className="text-[9px] bg-stone-100 text-stone-500 px-2 py-0.5 rounded font-black uppercase border">
                                                                {getCategoryNameTH(cat) ||
                                                                    '-'}
                                                            </span>
                                                        </div>
                                            ...
                                            <td className="py-3 px-4 text-right">
                                                <span className="font-black text-[15px] lg:text-[16px] text-[#861b00]">
                                                    ฿
                                                    {Number(
                                                        it.price || 0
                                                    ).toLocaleString()}
                                            ...
                                            <td className="py-3 pr-6 text-center">
                                                <div className="flex items-center justify-center gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                                    <button
                                                        ...
                                                        className="w-9 h-9 lg:w-10 lg:h-10 rounded-full text-emerald-500 hover:bg-emerald-50 flex items-center justify-center border bg-white shadow-sm"
                                                    >
                                                        <span className="material-symbols-outlined text-[18px]">
                                                            edit
                                                        </span>
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-9 h-9 lg:w-10 lg:h-10 rounded-full text-stone-300 hover:text-red-500 flex items-center justify-center border bg-white shadow-sm"
                                                    >
                                                        <span className="material-symbols-outlined text-[18px]">
                                                            delete
                                                        </span>
